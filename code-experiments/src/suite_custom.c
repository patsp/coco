...
static coco_suite_t *suite_custom_initialize(void) {
    ...
    suite = coco_suite_allocate("custom", 10, 100, dimensions,
                                "instances:1-15");
...
/**
 * @brief Implements the hyperbolic problem's objective function
 * (posed as minimization) without connections to any COCO structures.
 */
static double f_hyperbolic_raw(const double *x,
                              const size_t number_of_variables) {
 ...
 * @brief Uses the raw function to evaluate the COCO problem.
 */
static void f_hyperbolic_evaluate(coco_problem_t *problem,
                                 const double *x, double *y) {
  assert(problem->number_of_objectives == 1);
  y[0] = f_hyperbolic_raw(x, problem->number_of_variables);
  assert(y[0] + 1e-13 >= problem->best_value[0]);
}

/**
 * @brief Implements the hyperbolic problem's constraint
 * without connections to any COCO structures.
 */
static double f_hyperbolic_constraint_raw(const double *x,
                                         const size_t number_of_variables,
                                         const double *data) {
 ...
 * constraint threshold.
 */
static void f_hyperbolic_evaluate_constraint(coco_problem_t *problem,
                                            const double *x, double *y) {
    double cons = 0.0;
    assert(problem->number_of_constraints == 2);
    cons = f_hyperbolic_constraint_raw(x, problem->number_of_variables,
                                      (double *)problem->data);
    y[0] = cons - 1e-8;
    y[1] = -cons - 1e-8;
}
 ...
 * @brief Allocates the polygon problem.
 */
static coco_problem_t *f_hyperbolic_allocate(const size_t number_of_variables,
                                            const size_t function,
                                            const size_t instance) {
    size_t i = 0;
    size_t j = 0;
    ...
    coco_random_state_t *random_generator = coco_random_new(rseed);
    double x = 0.0;
    problem->evaluate_function = f_hyperbolic_evaluate;
    problem->evaluate_constraint = f_hyperbolic_evaluate_constraint;
    problem->initial_solution = coco_allocate_vector(number_of_variables);
    for (i = 0; i < number_of_variables; ++i) {
        /* Bounds are not relevant for this problem */
        problem->smallest_values_of_interest[i] = -5.0;
    ...
    coco_problem_set_id(problem, "hyperbolic");
...
/**
 * @brief Implements the parabolic problem's objective function
 * (posed as minimization) without connections to any COCO structures.
 */
static double f_parabolic_raw(const double *x,
                              const size_t number_of_variables) {
 ...
 * @brief Uses the raw function to evaluate the COCO problem.
 */
static void f_parabolic_evaluate(coco_problem_t *problem,
                                 const double *x, double *y) {
  assert(problem->number_of_objectives == 1);
  y[0] = f_parabolic_raw(x, problem->number_of_variables);
  assert(y[0] + 1e-13 >= problem->best_value[0]);
}

/**
 * @brief Implements the parabolic problem's constraint
 * without connections to any COCO structures.
 */
static double f_parabolic_constraint_raw(const double *x,
                                         const size_t number_of_variables,
                                         const double *data) {
 ...
 * constraint threshold.
 */
static void f_parabolic_evaluate_constraint(coco_problem_t *problem,
                                            const double *x, double *y) {
    double cons = 0.0;
    assert(problem->number_of_constraints == 2);
    cons = f_parabolic_constraint_raw(x, problem->number_of_variables,
                                      (double *)problem->data);
    y[0] = cons - 1e-8;
    y[1] = -cons - 1e-8;
}
 ...
 * @brief Allocates the polygon problem.
 */
static coco_problem_t *f_parabolic_allocate(const size_t number_of_variables,
                                            const size_t function,
                                            const size_t instance) {
    size_t i = 0;
    size_t j = 0;
    ...
    coco_random_state_t *random_generator = coco_random_new(rseed);
    double x = 0.0;
    problem->evaluate_function = f_parabolic_evaluate;
    problem->evaluate_constraint = f_parabolic_evaluate_constraint;
    problem->initial_solution = coco_allocate_vector(number_of_variables);
    for (i = 0; i < number_of_variables; ++i) {
        /* Bounds are not relevant for this problem */
        problem->smallest_values_of_interest[i] = -5.0;
    ...
    for (i = 0; i < number_of_variables / 2; ++i) {
        for (j = number_of_variables / 2; j < number_of_variables; ++j) {
            data[i * number_of_variables + j] = 0.0;
            data[j * number_of_variables + i] = 0.0;
        }
    }
    for (i = 0; i < number_of_variables; ++i) {
        if (i < number_of_variables / 2) {
            ...
            data[i * number_of_variables + i] = 0.0;
        }
    }
    problem->data = (void *)data;
    /* Provide an initial feasible solution. */
    ...
    coco_problem_set_id(problem, "parabolic");
...
static coco_problem_t *suite_custom_get_problem(coco_suite_t *suite,
    ...
    if (function == 1) {
        ...
        problem = f_hyperbolic_allocate(dimension, function, instance);
    } else if (function == 10) {
        problem = f_parabolic_allocate(dimension, function, instance);
    } else {
